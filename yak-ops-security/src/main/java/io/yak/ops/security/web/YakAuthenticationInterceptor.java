...
public class YakAuthenticationInterceptor implements HandlerInterceptor {
  ...
  public boolean preHandle(
    ...
    if ("OPTIONS".equalsIgnoreCase(request.getMethod()) || isPublicEndpoint(handler)) {
      return true;
    }
    ...
    return loginService.interceptorCheck(
        request, response, requestPath, properties.getPublicPaths());
  }
  ...
  private boolean isPublicEndpoint(Object handler) {
    ...
    return AnnotatedElementUtils.hasAnnotation(method.getMethod(), PublicEndpoint.class)
