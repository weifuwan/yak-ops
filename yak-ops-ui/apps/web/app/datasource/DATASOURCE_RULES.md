# Datasource Frontend Rules

Scope:
- `yak-ops-ui/apps/web/app/datasource/**`
- `yak-ops-ui/apps/web/service/datasource/**`

Owns:
- Datasource product UI and interaction
- Datasource model / presentation metadata
- Datasource editor / connection / plugin state
- Datasource backend endpoint adaptation

## Structure

```text
app/datasource/
├── connection/
├── editor/
├── i18n/
├── management/
├── model/
├── plugin/
└── index.tsx

service/datasource/
├── api.ts
├── catalog.ts
├── driver.ts
└── index.ts
```

## Ownership

`app/datasource`
- Datasource UI
- product state
- dynamic form runtime
- presentation metadata
- capability-local types and helpers

`service/datasource`
- CRUD endpoints
- connection test
- plugin config endpoints
- catalog endpoints
- driver upload

## Dependency Direction

```text
app/router
   ↓
app/datasource
   ↓
service/datasource
   ↓
service/http

app/datasource
   ↓
@yak-ops/yak-ui
```

## Must

- Datasource UI code stays under `app/datasource`.
- Datasource endpoint code stays under `service/datasource`.
- HTTP transport goes through `service/http`.
- Common UI primitives come from `@yak-ops/yak-ui`.
- Capability-local component / hook / type / helper stays near its owner.
- Dynamic form state stays in Datasource, not Yak UI.

## Must Not

- Recreate `packages/datasource`.
- Recreate `@yak-ops/datasource` alias or package dependency.
- Put Datasource-specific helpers into root `utils/hooks/types/constants`.
- Call `fetch` directly from Datasource UI.
- Recreate generic Button / Input / Select / Dialog primitives.
- Import App Router, App Layout or Auth Context into Datasource capability.
