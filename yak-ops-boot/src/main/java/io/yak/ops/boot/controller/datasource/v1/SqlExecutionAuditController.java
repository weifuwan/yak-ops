...
public class SqlExecutionAuditController {
    ...
    public Result<PagingData<SqlExecutionAuditVO>> page(
        ...
        return Result.success(auditConverter.page(auditReader.page(auditConverter.criteria(query))));
    }

    @Operation(summary = "查询 SQL 执行详情")
    @GetMapping("/{executionId}")
    public Result<SqlExecutionAuditDetailVO> detail(@PathVariable("executionId") String executionId) {
        return Result.success(auditConverter.detail(auditReader.detail(executionId)));
    }
    ...
    public Result<SqlExecutionAuditSummaryVO> summary(
            @Valid @RequestBody(required = false) SqlExecutionAuditQueryDTO query) {
        return Result.success(auditConverter.summary(auditReader.summary(auditConverter.criteria(query))));
    }
}
