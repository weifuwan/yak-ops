...
- Modal Popup uses a fast fade + subtle upward motion: open starts about 12px below and settles in about 140ms with a decelerating curve; close continues about 12px upward and fades out in about 120ms. Backdrop remains an opacity-only 200ms fade. Modal never uses scale, bounce or spring motion, and product code must not override Modal motion through `className`.
