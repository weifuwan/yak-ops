...
public final class LoginAttemptGuard {
  ...
  LoginAttemptGuard(
    ...
    if (properties == null) {
      throw new IllegalArgumentException(
              "login security properties must not be null");
    }
    ...
    if (properties.getMaxFailureCount() < 1) {
      throw new IllegalArgumentException(
              "max-failure-count must be greater than 0");
    }
  ...
  public boolean isBlocked(String username, String ipAddress) {
    ...
    return isBlocked(
            key("user", normalizeUsername(username)),
            now
    ) || isBlocked(
            key("ip", normalizeIp(ipAddress)),
            now
    );
  }
   ...
   */
  private static boolean isLockExpired(
          FailureState state,
          Instant now) {

    return state.blockedUntil != null
            && !now.isBefore(state.blockedUntil);
  }
