...
- Project Context behavior must remain fail-closed where current business behavior requires it.
...
- complex multi-table or explicit SQL semantics may use Mapper XML.
...
SQL Execution and Project Context code currently physically lives in this Maven module while some Java packages still use `io.yak.ops.core.*`.
...
Protect:
- CRUD business rules
- connection normalization and secret behavior
- catalog policy and mapping
- SQL execution lifecycle / cancellation / transaction behavior
- persistence semantics
- controller permission / contract
- plugin boundary behavior

Do not keep architecture tests that only assert deleted documentation or historical modules.
